...
**2026-05-03 — Calendar multi-day deploy + MEMORY:** Landed follow-up fixes ( **`centerDate`** refresh on calendar + view/mode, layout-aware **`jumpToTodayInstant`**, floating header sync, visibility → instant when ref supports it). **Ship:** **`git push origin main`** then **`npm run deploy:vercel:prod`** so **Git + Vercel** and local prebuilt agree; confirm **`npm run verify:prod-build`** after push. If prod still looks wrong, hard-refresh or **Application → Clear site data** (stale lazy chunk); grep prod **`CalendarEventsPage-*.js`** for **`jumpToTodayInstant`**.
