...
- **Nexus voice latency (~60s symptom):** Voice uses **`postNexusUserVoiceTurn`** → single **`fetch`**; STT can show text while the UI waits for **full** JSON. Server:**`vercel.json`** caps **`api/**/*.ts`** at **`maxDuration: 60`**; **`runNexusToolLoop`** allows up to **4** LLM rounds + tool execution + repair nudges — worst case can approach the cap. **Profile:** `localStorage.setItem('SYNCSCRIPT_VOICE_LATENCY','1')` reload → **`voice-latency-debug.ts`** marks + **`voiceLatencyLogNexusCorrelation`** (logs **`X-Nexus-Request-Id`** / brain version when flag on). **Immersive orb:** under-orb copy **“Nexus is thinking…”** while **`isProcessingAI`**; **`clearInterimTranscript`** on turn start so partial STT does not cover that line. Dated detail: **2026-04-11** entry below.
...
**Last updated:** 2026-04-17 — Dashboard weather **geo deadline** + **`useWeatherRoute` finally**; App AI voice **portal + z-index**; immersive voice **short intro** via **`generateImmersiveVoiceIntro`** (**`src/utils/voice-context-builder.ts`**). **2026-04-11** addendum: Nexus voice **~60s latency** investigation + **`MEMORY.md`** quick bullet ( **`postNexusUserVoiceTurn`**, **`maxDuration`**, tool loop). Earlier: Nexus **`src/config/nexus-vercel-ai-routes.ts`** (no prod-only `nexus-user` URLs); **`emitNexusTrace`** **`toolTraceEntries`** + **`toolRepairNudged`**; **`integrations/research/NEXUS_OBSERVABILITY_AND_QUALITY.md`**; **`api/cron/[job].ts`** **`concierge-playbook-tick`** → **`handleConciergePlaybookTickHttp`**. ROI ops: **`verify:prod-build`**, Lighthouse, skill-audit triage, **`gh:labels`**. Example eval: **`integrations/research/skill-evaluations/2026-04-17-playwright-mcp.md`**. **2026-04-16** Nexus: **CSP `frame-src`**, **`/api/map/resolve-map-url`**, **`update_document`** nudges (see **§ Nexus Voice**).
...
**Nexus “individual user” verify profile (agents / CI):** Run **`npm run bootstrap:nexus-verify-user`** once (needs **`SUPABASE_SERVICE_ROLE_KEY`** in `.env`). That **creates or resets** a dedicated Supabase Auth user and upserts **`NEXUS_LIVE_TEST_EMAIL`**, **`NEXUS_LIVE_TEST_PASSWORD`**, **`NEXUS_VERIFY_USER_ID`**, **`NEXUS_LIVE_TEST_SKIP_SERVICE_BOOTSTRAP=1`**. Then **`npm run verify:nexus-tools-live`** signs in with that profile (log line *Signing in with NEXUS_LIVE_TEST_EMAIL…*) and confirms **`create_task`** + **`GET /tasks`** for **that** user — not an ephemeral service-role bootstrap user. Default email is set in `scripts/bootstrap-nexus-individual-verify-user.mjs` (override with **`NEXUS_VERIFY_PROFILE_EMAIL`**). **Do not commit** `.env`; passwords are local only. **Playwright (browser):** same credentials — **`npm run test:e2e:nexus-task`** (create task); **`npm run test:e2e:nexus-signed-in-smoke`** (Edge **`user/profile`** 200 + **`propose_calendar_hold`** **`toolTrace`**); **`npm run test:e2e:nexus-app-ai-parity-deep`** (isolated sessions: **`create_task`**, **`propose_calendar_hold`**, **`create_document`**, optional **`search_places`**, immersive voice: overlay + orb + artifact rail). Optional env aliases **`E2E_LOGIN_EMAIL`** / **`E2E_LOGIN_PASSWORD`**. **On GitHub Actions only:** **`search_places`** and **voice** are skipped unless repository secrets **`NEXUS_E2E_INCLUDE_PLACES=1`** / **`NEXUS_E2E_INCLUDE_VOICE=1`** (flaky **`nexus-user`** / mic + lazy chunk on runners). Local runs include places unless **`NEXUS_E2E_SKIP_PLACES=1`**. **GitHub Actions:** workflow **`e2e-nexus-signed-in-prod.yml`** (`workflow_dispatch`) — set secrets **`NEXUS_LIVE_TEST_EMAIL`** / **`NEXUS_LIVE_TEST_PASSWORD`** (or **`E2E_LOGIN_*`** duplicates). **`enqueue_playbook` / concierge** not in Playwright yet (needs stable playbook slug + worker); contract tests cover wiring.
