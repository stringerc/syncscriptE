...
**2026-05-03 — Calendar multi-day deploy + MEMORY:** Landed follow-up fixes ( **`centerDate`** refresh on calendar + view/mode, layout-aware **`jumpToTodayInstant`**, floating header sync, visibility → instant when ref supports it). **Shipped:** commit **`297d5b7`** on **`main`** (`stringerc/syncscriptE`), **`git push`** (prepush: **`npm test`** + **`verify:console-errors`**), **`npm run deploy:vercel:prod`**. **`npm run verify:prod-build`:** **MATCH** **`297d5b7`** on **`www.syncscript.app`**. Lazy chunk example: **`/assets/CalendarEventsPage-Ax4SYca9.js`** (hash varies by build). If the UI still looks cached, **hard refresh** or clear **syncscript.app** site data (immutable **`/assets/*`**).
