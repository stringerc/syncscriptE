...
6. **CI workflow:** **`.github/workflows/e2e-nexus-signed-in-prod.yml`** — name **“E2E Nexus signed-in (prod)”**. Trigger: **Actions → Run workflow**, or CLI: **`gh workflow run "E2E Nexus signed-in (prod)"`**. Requires secrets from step 3. **On GitHub Actions only:** **`search_places`** / **voice** skipped unless secrets **`NEXUS_E2E_INCLUDE_PLACES`** / **`NEXUS_E2E_INCLUDE_VOICE`** are **`1`** (flaky **`nexus-user`** / runner mic).
