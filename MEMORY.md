...
- **Idle CPU / hidden tab (2026-04-26):** If marketing or dashboard still feels hot, use **Chrome DevTools → Performance**, **CPU 4× slowdown**, record **~10s** on **`/`** and **`/dashboard`**, and inspect **Main** for long **`requestAnimationFrame`** / **`Timer fired`** stacks. Playbook + file pointers: **`integrations/research/PERF_IDLE_CPU_PROFILING.md`**. **Shipped guards:** **`ScrollAmbience`** (visibility + ~30fps cap; unused today), calendar **`CurrentTimeLine`** / **`WeekView`** intervals skip work when **`document.hidden`**, **`NexusVoiceCallContext`** typewriter skips ticks when hidden + call duration from **wall clock** (not interval deltas). **PWA:** **`vite.config.ts`** **`workbox.globIgnores`** may exclude heavy optional chunks (e.g. Three vendor) from **precache** — install size, not steady FPS. **`ContinuityContext`** heartbeat stays **protected** — profile before any change proposal.
- **Dashboard visual-quality gate (2026-04-26):** **`npm run test:e2e:visual-quality`** — Playwright build + preview: three-column **geometry** (`#ai-suggestions`, `#energy-meter`, `#roygbiv-ring`), small **`toHaveScreenshot`** baselines (`/`, guest login shell, guest `/dashboard` chat closed), **Axe** on `[data-section=hero]` + guest `#main-content`; **Firefox** runs layout spec only. Refresh PNGs: **`npm run test:e2e:visual-snapshots:update`**. Post-deploy: **`npm run verify:post-deploy:checklist`** (prod HTML marker vs local git + OG/JSON-LD + **`/.well-known/security.txt`**). CI: **`.github/workflows/ci.yml`** job **`visual-quality-e2e`**. RUM SLO notes: **`integrations/research/RUM_SLO_SYNCSCRIPT.md`**.
...
**2026-04-26 (prod ops — same day):** **`supabase db push`** applied **`user_onboarding_progress`** migration to **`kwhnrlzibgfedtxpkbgb`**. **`npm run deploy:edge:make-server`** shipped Stripe interval + annual SKU resolver. **`npm run deploy:vercel:prod`** completed; **`npm run verify:prod-build`** → **MATCH** (`www.syncscript.app` vs local git). Spot-check in browser: **`/trust`**, **`/changelog`**, **`/docs/api`**, **`/openapi.json`**, **`/.well-known/security.txt`**.

**2026-04-26 — Calendar page: day timeline + sidebar rail:** Product requirement: the **320px rail** stays **to the right** of the day/week views. Layout is a **`flex-row` + `shrink-0` rail** (see quick-section **Calendar `/calendar` layout**); **`e2e/dashboard-layout-columns.spec.ts`** asserts geometry. Files: **`CalendarEventsPage.tsx`**, **`PrecisionDayView.tsx`**.

**2026-04-26 (evening) — git tree + Edge completeness:** `main` had **months of uncommitted Tier 0–1 work** plus **local-only noise** (Aider, Playwright screenshots, **`nature-cortana-platform/`**, root `PHASE_*` / `EX*` planning MD). Landed commits **`920c0af`** (full product + dashboard `md` three-column + API + integrations + tests), **`a654c7d`** (Edge **`hermes-bridge.tsx` / `engram-bridge.tsx`** and related routes were **missing from git** while `index.tsx` imported them — fixed), **`deb745e`** / **`0372dce`** (gitignore + SESSION). **`git status`** clean; **`npm run verify:prod-build`** **MATCH** after **`npm run deploy:vercel:prod`**.

**2026-04-26 (resume) — post-crash verification:** Fixed **`ApiDocsPage`** RapiDoc CDN URL (was a broken placeholder). **`security.txt`** + **`openapi.json`** contact lines use real **`security@`** addresses. **`TrustPage` / `ChangelogPage`** import **`Link`** from **`react-router`** (matches **`App.tsx`**). **`hasLoggedEnergy`**: **`AuthContext`** merges from **`user_onboarding_progress`** after profile fetch + listens for **`syncscript:onboarding-progress-synced`** (dispatched from **`checklist-tracking`** after successful upsert) — no **`EnergyContext`** edit (protected). **Agent runner live:** default is now **strict** (fetch failure exits 1); set repo Variable **`AGENT_RUNNER_LIVE_RELAXED=1`** during bring-up (or legacy **`AGENT_RUNNER_LIVE_VERIFY=0`**). **Chromatic:** token present no longer runs broken **`chromaui/action`** without Storybook. **CI:** Lighthouse runs against **`vite preview`** after build; App AI E2E can be hard-required with **`REQUIRE_APP_AI_E2E=1`** when secrets exist. **Full-repo `tsc`:** not green yet (legacy **`sonner@2.0.3`** imports, etc.) — CI emits a **notice** instead of blocking until cleaned up; **`npm run build`** + ESLint still gate. Fixed **`card-enhancements/*.tsx`** accidental **`className=\"...\"`** escapes; moved MIT text from **`src/LICENSE/main.tsx`** → **`src/LICENSE/MIT.txt`**; **`src/tsconfig`** excludes **`LICENSE/**`** + **`vite.config.ts`**. Contract test updated for Tier-A **`extract_links`** in **`safety.mjs`**. Verified **`npm test`** + **`CI=true npm run build`** + bundle-size script locally.

**2026-04-26 — Tier 0 + Tier 1 strategic audit pass:** Shipped the audit roadmap items that were entirely in our control (no external accounts needed). **Tier 0 A — onboarding correctness:** new server-tracked **`user_onboarding_progress`** table (RLS-scoped, realtime-published) + **`pullOnboardingProgressFromServer`** in **`src/components/onboarding/checklist-tracking.ts`** hydrates on auth-ready via **`ObservabilityIdentifier`**; existing call sites unchanged. SignupPage now navigates to `/onboarding` (matches OAuth flow). **`isFirstTime`** in **`DashboardPage`** is **`useState(() => …)`** so it can't flip mid-render. Deleted unused **`ProductTour.tsx`** + uninstalled `react-joyride`. **Tier 0 B — billing wiring:** **`BillingSettings`** mounted in Settings → Billing tab; **`BillingPlansModal.handleSelectPlan`** routes to Stripe Customer Portal (existing subs) or Checkout (new subs) instead of local-profile-only **`updateProfile`**; PricingPage passes **`interval: 'month' | 'year'`**; **`stripe-routes.tsx`** has **`resolvePriceId`** + env **`STRIPE_PRICE_<plan>_YEAR`** for annual SKUs (falls back to monthly + echoes `actual_interval` when annual not configured). Entitlements reconciled — **`SubscriptionContext`** imports **`LITE_TIER_LIMITS`** from **`utils/entitlement-contract.ts`** (single source = 5/day). **`<PastDueBanner />`** mounted in App for `past_due` / `unpaid` / `incomplete` states. **Tier 0 C — CI gates:** **`npm test`** (105 contract tests) added to **`.github/workflows/ci.yml`** as the new **`contract-tests`** job; bundle-size budget via **`scripts/check-bundle-size.mjs --max-kb=900`**; Lighthouse Perf flipped from `warn` → `error` in **`lighthouserc.cjs`**. **Tier 1 F — `/trust` portal:** **`TrustPage.tsx`** + **`public/.well-known/security.txt`** with subprocessors, encryption, GDPR/CCPA, residency, security disclosure path. **Tier 1 G — `/changelog`:** **`ChangelogPage.tsx`** reads **`public/changelog.json`** (deploy-time-editable, no rebuild required). **Tier 1 H — help widget:** **`HelpWidget.tsx`** boots Crisp / Intercom when **`VITE_HELP_WIDGET_PROVIDER`** + provider id env are set; no-op otherwise. CSP updated for both. **Tier 1 I — `/docs/api`:** **`ApiDocsPage.tsx`** uses RapiDoc CDN viewer pointed at **`public/openapi.json`** (hand-curated for `/api/agent/*` + BYOK + Webhooks + Policy + Live). **Tier 2 N — Chromatic stub:** **`.github/workflows/chromatic.yml`** that auto-skips when `CHROMATIC_PROJECT_TOKEN` secret is absent. **Tier 2 O — CWV pipeline:** **`web-vitals`** dep + **`src/observability/web-vitals.ts`** emits `cwv_lcp/inp/cls/fcp/ttfb` events to PostHog with `device` cohort. **Defense-in-depth:** **`useEmotionDetection`** rAF loop now gated on **`usePageVisibility().isVisible`** so prosody polling stops on hidden tabs (Chrome's 1Hz throttle isn't the same as zero CPU). **Deferred (need user action):** quota WRITER on task creation (Tier 0 B 6th item — backend choke point design); **D Better Uptime** + **E Vanta SOC 2** + **H Crisp/Intercom account** + **L Google Calendar OAuth client** + **M Apple Dev creds for TestFlight** + **K streaming Nexus voice** (touches `02-protected-files`). For Tier 0 B annual SKUs: create annual prices in Stripe, set Vercel env **`STRIPE_PRICE_STARTER_YEAR`** / **`_PROFESSIONAL_YEAR`** / **`_ENTERPRISE_YEAR`** — code path is already wired. New Edge migration needs **`supabase db push`**.
...
## Product positioning — AI integration vs replacement (curated 2026)

**Yes — this is what syncscript.app is about:** a **work, calendar, tasks, and voice** surface where **models are swappable infrastructure** (`api/_lib/ai-service.ts` — Groq, NVIDIA, Anthropic, OpenAI, …), not the product’s sole identity. Anthropic and peers win the **general intelligence + distribution** race; SyncScript wins a **slice** only if it stays disciplined on that slice.

### What SyncScript is (operator + founder framing)

- **Not a Claude replacement.** Users keep using **Claude, ChatGPT, Cursor, etc.** where those tools excel. SyncScript is the **place those capabilities meet your real week**: tasks, calendar, energy, billing-adjacent workflows, team, scripts—**one app**, **your** Supabase-backed state, **bounded** Nexus tools + traces (`emitNexusTrace`, contracts in `npm test`).
- **Integration layer as AI shifts.** Models and vendors change monthly; SyncScript’s bet is **multi-provider routing + workflows** so you are **not locked to one lab** to get work done inside the product. “Use the best model available **with** SyncScript” beats “be the only model.”
- **Proprietary value = energy + continuity.** The durable differentiator is **energy-aware scheduling** and **readiness-aligned orchestration** (dashboard, Today, AI Focus, marketing demo)—**when** to lean on heavy AI vs light work, not raw benchmark scores. Keeping “up with AI” here means **wiring new providers and tools into the same loop**, not rebranding as the next chatbot.

### What Anthropic-scale players will not optimize for (your moat to defend)

- **Circadian / energy as first-class state** tied to **task + calendar** UX—not a generic assistant sidebar.
- **Deep parity**: “Anything the web app allows” via Nexus on **voice, in-app chat, phone** where feasible (MEMORY quick section)—**completeness** over generic answers.
- **Trust in a vertical**: playbook tiers, OAuth, audit trails, Edge tasks—**constrained behavior in your domain**, not only model-level safety.

### Honest constraint

**Breadth vs depth:** the full feature catalog is wide; the positioning above only holds if the **spine** (energy + calendar + tasks + Nexus) stays visibly best-in-class. Bundled Big Tech assistants remain the default competitor—**narrow story + measurable retention** beats “we have more tabs.”
...
- **Deploy:** migration `20260427090000_activity_business_plan_api_tokens.sql` then **`npm run deploy:edge:make-server`** (redeploy after Edge rate-limit / activity changes). **Entry file is `supabase/functions/make-server-57781ad9/index.ts`** (not `index.tsx`) — `social-productivity-routes` + **`financial-routes`** must be imported and `app.route`’d there or production will 404 those paths; guarded by **`tests/edge-social-productivity-mount-contract.test.mjs`**. After deploy: **`npm run verify:edge-productivity-http`** (optional **`SYNCSCRIPT_TEST_JWT`** for authenticated rows). **Signed-in UI + Edge regression:** **`npm run test:e2e:signed-in-productivity`** (`e2e/signed-in-productivity-edge.spec.ts`) with **`E2E_LOGIN_*`** or **`NEXUS_LIVE_TEST_*`** in `.env` — heatmap `GET /activity/summary`, PAT create/revoke, friend feed `GET /friends/activity-feed`. Cursor conversations do **not** auto-append to MEMORY — curate manually or use `memory/YYYY-MM-DD.md` + hooks (see spec).
- **Backlog:** automatic **`calendar_event_done`** from a single calendar save path — still **deferred** (see spine § Repo completion notes).

### MCP operating model (external AIs + syncscript.app)

- **Source of truth:** Signed-in **syncscript.app** (dashboard) and **Supabase Edge** `make-server-57781ad9` for tasks, calendar holds, capture inbox, activity, business plan, **user file library** (`/resources/*`), and profile — same routes the React app uses via JWT; **Cursor / Claude / any MCP host** uses a **scoped PAT** (`sspat_…`) with explicit scopes (rotate PATs to pick up new scopes such as **`library:read`** / **`library:write`**).
- **Planning loop (recommended):** In the IDE, call **`syncscript_week_snapshot`** once → tasks + `calendar/local-events` + activity window + capture inbox → draft **`syncscript_put_business_plan`** / tasks / **`syncscript_calendar_hold`** / **`syncscript_create_capture_inbox_item`** (human gate) → **`syncscript_log_activity`** (`external_ide_session` / `focus_block`) so the heatmap reflects real work.
- **Files from agents:** **`syncscript_library_upload_base64`** → Edge **`POST /resources/upload-json`** (≤ **1 MiB** decoded per file; larger binaries → signed-in app **multipart** `POST /resources/upload` up to **50 MiB**). Link to a task with **`syncscript_library_link_file`** (`entity_type: task`, `entity_id: <task id>`). **`email-self`** remains **JWT-only** (PAT cannot resolve mailbox for Resend).
- **Not on MCP yet (client-first):** **Goals** (`useGoals` / local command adapter), **Workstream** canvas graph, and **friend feed** stay in-app or JWT-only APIs — see **`integrations/research/MCP_PARITY_AND_ROADMAP.md`** for phased server alignment if we want full “GitHub + calendar” parity from externals.
- **Safety:** Never commit PATs; split tokens per machine; use **capture inbox** for risky bulk writes; keep exhibits and secrets out of agent prompts — **links** in task descriptions + **library** storage.
...
- **What:** `api/_lib/openai-compat-model-policy.ts` centralizes model-specific Chat Completions JSON (strip tool `is_error` for **Kimi** / `kimi-*`, `max_completion_tokens` + no classic sampling for **o1/o3/o4** and **gpt-5**-family heuristics). Applied in **`api/_lib/ai-service.ts`** (Nexus user/guest/phone platform LLM) and **`api/_lib/agent-llm-adapter.ts`** `callAgentChat` (BYOK + runner).
- **Diagnostics:** `GET /api/agent/llm-stack` (auth) returns provider chain + which env keys are present (no values), runner env flags, executor-bridge config flags.
- **Gateway bridge:** Optional `NEXUS_EXECUTOR_BRIDGE_URL` + `NEXUS_EXECUTOR_BRIDGE_SECRET` (+ optional `NEXUS_EXECUTOR_BRIDGE_INVOKE_PATH`, default `/v1/invoke`). `GET /api/agent/executor-bridge` probes health; `POST` forwards JSON with `Authorization: Bearer` + `X-SyncScript-User-Id` — same *spirit* as Edge `hermes-bridge`, but server-to-server from Vercel for a self-hosted OpenClaw/Hermes/custom worker.
- **BYOK order:** Multiple keys → **`groq` before `openrouter` / `anthropic`** in `agent-llm-adapter.ts`.
- **Agent platform LLM (no BYOK):** Not NVIDIA-only — first available env among `NVIDIA_API_KEY`, `GROQ_API_KEY`, `ANTHROPIC_API_KEY`, `OPENROUTER_API_KEY`, `GEMINI_API_KEY`, `OPENAI_API_KEY`, `XAI_API_KEY`, `MISTRAL_API_KEY` (`PLATFORM_LLM_FALLBACK_ORDER`). Oracle runner **`deploy/nexus-agent-runner/runner/llm.mjs`** matches the same fallback + BYOK priority as the adapter.
- **Startup credits playbook:** `integrations/research/STARTUP_CLOUD_CREDITS_PLAYBOOK.md` (AWS Activate / Google for Startups / Microsoft — official links; human must submit).
- **Doc:** `integrations/research/NEXUS_LLM_COMPAT_AND_EXECUTOR_BRIDGE.md` · tests `tests/nexus-openai-compat-policy.test.ts` + extended `agent-mode-foundation-contract`.
...
| **Nexus tools live verify** | **Runbook (GitHub secrets + triage):** `.github/docs/NEXUS_TOOLS_LIVE_VERIFY.md`. Command: `npm run verify:nexus-tools-live`. CI: `.github/workflows/nexus-tools-live.yml` maps `ENGRAM_LIVE_USER_JWT` → `NEXUS_LIVE_TEST_JWT`; preflight fails fast if no `GROQ_API_KEY`/`NVIDIA_API_KEY`. Local JWT/bootstrap: env vars, or **Supabase CLI** `supabase projects api-keys --project-ref kwhnrlzibgfedtxpkbgb` (**service_role** never in git/MEMORY). Action logs: search **`[verify] FAIL:`**. |
...
- **Modal Layering & Deep Blurs (React Portal Mount):**
  - Mounted the modal inside an SSR-safe **React Portal** attached directly to `document.body` in [DailyOpsModal.tsx](file:///Users/Apple/syncscript/src/components/DailyOpsModal.tsx) to completely escape the header's `z-10` stacking context.
  - Upgraded backdrop overlay to `bg-black/75 backdrop-blur-md z-[10000]` to ensure absolute full-page coverage, perfectly obscuring and blurring all background components (such as the sidebar, header profile menu, weather, and AI focus energy cards) underneath.
  - Designed premium frosted glassmorphism for the modal container using `bg-gradient-to-b from-[#0f1620]/60 to-[#0a0e18]/60 backdrop-blur-xl z-[10001] shadow-2xl`, achieving magnificent visual depth.
- **Harmony Brief Background Preloading:**
  - Configured automatic **background preloading** in `DailyOpsModal.tsx`. As soon as the user's authenticated `accessToken` is ready, it fetches the briefing and stores it in the React state *before* the user ever clicks the briefing button.
  - Slashes modal load time from ~30s+ to **0ms (instantaneous)** on user click, providing a premium, zero-latency opening experience.
