        ...
        .find_map(|(alias, metadata)| {
            (*alias == lower).then_some(match metadata.provider {
                ...
                ProviderKind::Nvidia => match *alias {
                    "llama" | "llama-3.3" => "meta/llama-3.3-70b-instruct",
...
pub fn metadata_for_model(model: &str) -> Option<ProviderMetadata> {
    ...
    // NVIDIA NIM models: explicit nvidia/ prefix, or meta/* (Llama family),
    // mistralai/* (Mistral on NIM), or bare llama-* names.
...
const FOREIGN_PROVIDER_ENV_VARS: &[(&str, &str, &str)] = &[
    ...
    (
        "NVIDIA_API_KEY",
        "NVIDIA NIM",
        "use a NIM model alias (e.g. `--model llama`) or a namespaced model (e.g. `--model meta/llama-3.3-70b-instruct`) so prefix routing selects the NVIDIA NIM backend",
    ),
];
