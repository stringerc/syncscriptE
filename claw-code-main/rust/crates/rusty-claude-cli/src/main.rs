...
fn validate_model_syntax(model: &str) -> Result<(), String> {
    ...
    match trimmed {
        "opus" | "sonnet" | "haiku" | "llama" | "llama-3.3" | "llama-3.2" | "grok"
        | "grok-3" | "grok-mini" | "grok-3-mini" | "grok-2" | "kimi" => return Ok(()),
        _ => {}
    }
    ...
    // Check provider/model format: provider_id/model_id (allow nested like meta/llama-3.3-70b-instruct)
    if trimmed.contains('/') {
        let slash = trimmed.find('/').unwrap();
    ...
    // Bare model names that map to a known provider via prefix routing
    if trimmed.starts_with("llama-")
        || trimmed.starts_with("claude-")
        || trimmed.starts_with("grok-")
        || trimmed.starts_with("qwen-")
        || trimmed.starts_with("kimi-")
    {
        return Ok(());
    }
    let mut err_msg = format!(
        "invalid model syntax: '{}'. Expected provider/model (e.g., anthropic/claude-opus-4-6, meta/llama-3.3-70b-instruct) or known alias (opus, sonnet, haiku, llama, grok, kimi)",
        trimmed
    );
...
impl AnthropicRuntimeClient {
    fn new(
        ...
        let client = match detect_provider_kind(&resolved_model) {
            ...
            ProviderKind::Xai | ProviderKind::OpenAi | ProviderKind::Nvidia => {
                // The api crate's `ProviderClient::from_model_with_anthropic_auth`
                // with `None` for the anthropic auth routes via
                // `detect_provider_kind` and builds an
                // `OpenAiCompatClient::from_env` with the matching
                // `OpenAiCompatConfig` (openai / xai / dashscope / nvidia).
                ...
                // other OpenAI-compat endpoint users configure via their
                // respective BASE_URL env vars.
