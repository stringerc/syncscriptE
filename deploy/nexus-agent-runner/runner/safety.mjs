...
export function gate({ tier, action, currentUrl, trustedSites, blockedSites }) {
  // Step 1: blocked sites are always denied.
  if (currentUrl && siteIsBlocked(currentUrl, blockedSites)) {
    return { decision: 'block', reason: `site_blocked:${safeHost(currentUrl)}` };
  }
