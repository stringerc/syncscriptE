...
export async function processIncomingEmail(c: Context) {
  try {
    ...
    // If should auto-send, send immediately
    if (autoResponse.shouldAutoSend) {
      console.log(`[Auto-Responder] Auto-sending Tier 1 response (confidence: ${autoResponse.confidence})`);
      
      // Schedule follow-ups if needed
      if (autoResponse.followUpScheduled) {
        await scheduleFollowUp(id, 2, autoResponse.followUpScheduled);
        console.log(`[Auto-Responder] Scheduled Tier 2 follow-up for ${autoResponse.followUpScheduled}`);
      }
    }
    ...
    return c.json({
      ...
      autoResponse,
      detection,
      message: autoResponse.shouldAutoSend ? 'Auto-response sent' : 'Escalated to human'
    });
