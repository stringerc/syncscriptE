...
export interface AIInsightsContent {
  ...
  visualizations?: {
    type: 'radar' | 'comparison' | 'metrics' | 'heatmap' | 'trend';
    data: any;
    label?: string;
  }[];
}
...
export const AIInsightsSection = memo(function AIInsightsSection({ isOpen, content }: AIInsightsSectionProps) {
  ...
  // Debug: Log visualizations
  useEffect(() => {
    if (displayContent.visualizations && displayContent.visualizations.length > 0) {
      console.log('AI Insights Visualizations:', displayContent.visualizations);
    }
  }, [displayContent]);
    ...
    <div className="h-full flex flex-col bg-[#1a1c20]">
      ...
      <div className="flex items-center justify-center px-4 py-4 border-b border-gray-800 relative">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-white">{displayContent.title || 'AI Insights'}</h2>
        </div>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56 bg-[#1e2128] border-gray-800">
                <DropdownMenuLabel className="text-white">AI Insights Settings</DropdownMenuLabel>
