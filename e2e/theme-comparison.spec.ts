...
test('capture theme screenshots', async ({ page }) => {
  ...
  await page.screenshot({ path: 'test-results/theme-dashboard.png', fullPage: false });

  await page.goto('/tasks', { waitUntil: 'domcontentloaded' });
  await page.waitForTimeout(2000);
  await page.screenshot({ path: 'test-results/theme-tasks.png', fullPage: false });

  await page.goto('/ai', { waitUntil: 'domcontentloaded' });
  await page.waitForTimeout(2000);
  await page.screenshot({ path: 'test-results/theme-ai-tab.png', fullPage: false });
});
