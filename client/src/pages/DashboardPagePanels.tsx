...
export function DashboardPage() {
  ...
  useEffect(() => {
    if (isHydrated) {
      telemetryService.record('ui.panel.rendered', { 
        screen: 'dashboard', 
        panel: 'ai-focus' 
      });
      telemetryService.record('ui.panel.rendered', { 
  ...
  return (
    <div className="space-y-6">
      ...
      <Panel>
        <PanelHeader>
          <div>
            <PanelTitle>AI & Focus</PanelTitle>
            <PanelSubtitle>Your personalized insights and recommendations</PanelSubtitle>
          </div>
          <Toolbar actions={[
            { icon: 'Brain', label: 'Generate Plan', onClick: () => console.log('Generate plan') },
            { icon: 'Sparkles', label: 'Prioritize', onClick: handlePrioritizeTasks }
          ]} />
        </PanelHeader>
        <PanelBody>
          {standardizedTasks.length === 0 ? (
            <EmptyState
              title="No tasks to prioritize"
              description="Add some tasks to get AI-powered prioritization and insights."
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="text-center p-4 rounded-lg bg-muted/50">
                <div className="text-2xl font-bold text-brand-primary">{standardizedTasks.length}</div>
                <div className="text-sm text-muted-foreground">Total Tasks</div>
              </div>
              <div className="text-center p-4 rounded-lg bg-muted/50">
                <div className="text-2xl font-bold text-brand-accent">{completedTasksCount}</div>
                <div className="text-sm text-muted-foreground">Completed</div>
              </div>
              <div className="text-center p-4 rounded-lg bg-muted/50">
                <div className="text-2xl font-bold text-green-600">{activeStreaks.length}</div>
                <div className="text-sm text-muted-foreground">Active Streaks</div>
              </div>
              <div className="text-center p-4 rounded-lg bg-muted/50 cursor-pointer hover:bg-muted/70 transition-colors" onClick={() => navigate('/gamification')}>
      ...
      <Panel>
        <PanelHeader>
          <PanelTitle>Pinned Events</PanelTitle>
          <PanelSubtitle>Your most important events at a glance</PanelSubtitle>
        </PanelHeader>
        <PanelBody>
          <PinnedEventsRail />
        </PanelBody>
      ...
      <Panel>
        <PanelHeader>
          <div>
            ...
            <PanelSubtitle>Your tasks and mini-calendar for today</PanelSubtitle>
          </div>
          <Toolbar actions={[
            { icon: 'Plus', label: 'Add Task', onClick: handleAddTask },
            { icon: 'Calendar', label: 'Add Event', onClick: handleAddEvent }
          ]} />
        </PanelHeader>
        <PanelBody>
            ...
            <div>
              ...
              {standardizedTasks.length === 0 ? (
                ...
                <div className="space-y-3">
                  {standardizedTasks.slice(0, 5).map((task, index) => (
                    <TaskItem
                      key={task.id}
                      task={task}
            ...
            <div>
              ...
              {safeUpcomingEvents.length === 0 ? (
                <EmptyState
                  ...
                  description="Schedule an event to stay organized."
      ...
      <Panel>
        <PanelHeader>
          <div>
            <PanelTitle>Resource Hub</PanelTitle>
            <PanelSubtitle>Budgets, conflicts, and energy management</PanelSubtitle>
          </div>
          <Toolbar actions={[
            { icon: 'DollarSign', label: 'Connect Budget', onClick: () => navigate('/budgeting') },
            { icon: 'AlertCircle', label: 'View Conflicts', onClick: () => navigate('/calendar') }
          ]} />
        </PanelHeader>
        <PanelBody>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-4 rounded-lg border border-border/50">
              ...
              <div className="space-y-2">
                <div className="text-sm text-muted-foreground">No budgets configured</div>
                <Button size="sm" variant="outline" onClick={() => navigate('/budgeting')}>
                  Connect Budget
                </Button>
              </div>
            </div>
              ...
              <div className="space-y-2">
                <div className="h-2 w-full rounded-full bg-muted">
                  <div 
                    ...
                    style={{ width: `${(gamificationData?.stats?.currentLevel || 1) * 10}%` }}
                  />
                </div>
                <div className="text-sm text-muted-foreground">
                  Level {gamificationData?.stats?.currentLevel || 1}
                </div>
              </div>
            </div>
      ...
      {recentAchievements.length > 0 && (
        <Panel>
          <PanelHeader>
            ...
            <PanelSubtitle>Your latest accomplishments and milestones</PanelSubtitle>
          </PanelHeader>
          <PanelBody>
