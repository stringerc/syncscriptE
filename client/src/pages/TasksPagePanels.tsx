...
export function TasksPage() {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      // "n" or Alt+N to open new task modal
      if ((event.key === 'n' || (event.altKey && event.key === 'n')) && !isInputFocused) {
        event.preventDefault();
        lastActiveElementRef.current = document.activeElement as HTMLElement;
        setSelectedTask(null);
        setIsTaskModalOpen(true);
        telemetryService.record('ui.drawer.opened', { 
          screen: 'tasks', 
          kind: 'task-create' 
        });
      }
  ...
  const handleCloseTaskModal = () => {
    ...
    // Emit telemetry for drawer close
    telemetryService.record('ui.drawer.closed', { 
      screen: 'tasks', 
      kind: selectedTask ? 'task-edit' : 'task-create' 
    });
  }
        ...
        <Panel density="compact">
          <PanelHeader>
            <div>
              ...
              <PanelSubtitle>Manage your tasks with AI-powered prioritization</PanelSubtitle>
            </div>
            <Toolbar actions={[
              { 
                icon: 'Plus', 
        ...
        <Panel>
          ...
          <PanelBody>
            {pendingTasks.length === 0 ? (
              <EmptyState
                title="No tasks!"
                description={completedTasks.length > 0 ? 'All tasks completed! 🎉' : 'Create your first task to get started.'}
        ...
        isNewUI ? (
          <Panel>
            <PanelHeader>
              ...
              <Toolbar actions={[
                { 
                  icon: showCompletedTasks ? 'EyeOff' : 'Eye', 
                  label: showCompletedTasks ? 'Hide' : 'Show', 
        ...
        isNewUI ? (
          <Panel tone="warning">
            <PanelHeader>
              ...
              <Toolbar actions={[
                { 
                  icon: showDeletedTasks ? 'EyeOff' : 'Eye', 
                  label: showDeletedTasks ? 'Hide' : 'Show', 
