...
// - Day 0: Immediate activation (86% open rate — GetResponse 2024)
// - Day 3: Feature discovery → 67% retention if 3+ features tried (Intercom)
// - Day 7: Midpoint stats → 31% churn reduction (Pendo 2024)
// - Day 10: Urgency window → 2.7x conversion in last 5 days (ConversionXL)
// - Day 13: Final day → 4.2x conversion vs mid-trial (Baremetrics 2024)
export const REVERSE_TRIAL_SEQUENCE: EmailCampaign = {
  id: 'reverse_trial_sequence',
  name: 'Reverse Trial 14-Day Drip',
  type: 'drip',
  status: 'active',
  segment: 'reverse_trial',
  emails: [
    {
      template_name: 'reverse_trial_welcome',
      delay_hours: 0, // Immediate
    },
    {
      template_name: 'reverse_trial_day3',
      delay_hours: 72, // Day 3 — feature discovery
    },
    {
      template_name: 'reverse_trial_day7',
      delay_hours: 168, // Day 7 — midpoint stats
    },
    {
      template_name: 'reverse_trial_day10',
      delay_hours: 240, // Day 10 — urgency begins
    },
    {
      template_name: 'reverse_trial_final',
      delay_hours: 312, // Day 13 — final day warning
    },
  ],
  stats: { sent: 0, opened: 0, clicked: 0, bounced: 0, unsubscribed: 0 },
  created_at: new Date().toISOString(),
...
export async function getPendingEmails(): Promise<EmailSchedule[]> {
  ...
  const batchIds = pendingQueue.slice(0, MAX_BATCH_SIZE);
  
  for (const id of batchIds) {
    const schedule = await getScheduledEmail(id);
    
    if (!schedule) {
      // Missing schedule - remove from queue
      idsToRemove.push(id);
      continue;
    }
    ...
    if (schedule.status === 'pending') {
      ...
      // Already processed/failed/cancelled - remove from queue
      idsToRemove.push(id);
    }
  }
  ...
  if (idsToRemove.length > 0) {
    const cleanedQueue = pendingQueue.filter(id => !idsToRemove.includes(id));
    await kv.set('email_pending_queue', cleanedQueue);
    console.log(`[Email Automation] Cleaned ${idsToRemove.length} processed items from queue`);
  }
...
export async function processScheduledEmails(resendApiKey: string): Promise<number> {
  try {
    ...
    let skipped = 0;
    let failed = 0;
    const processedIds: string[] = []; // Track IDs to remove from queue
    ...
    // Remove all processed/failed/cancelled emails from queue
    if (processedIds.length > 0) {
      const pendingQueue = (await kv.get('email_pending_queue') || []) as string[];
      const cleanedQueue = pendingQueue.filter((id: string) => !processedIds.includes(id));
      await kv.set('email_pending_queue', cleanedQueue);
      console.log(`[Email Automation] Removed ${processedIds.length} processed emails from queue`);
    }
...
export async function initializeEmailSystem(): Promise<void> {
  ...
  await saveCampaign(REVERSE_TRIAL_SEQUENCE);
  
  console.log('[Email Automation] System initialized (beta + reverse trial sequences)');
}
